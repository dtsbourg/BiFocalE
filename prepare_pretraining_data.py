...
"""Create masked LM/next sentence masked_lm TF examples for MAGRET."""
...
class TrainingInstance(object):
  ...
  def __str__(self):
    ...
    s += "adjacency_len: %s\n" % (" ".join(
        [tokenization.printable_text(x) for x in self.adjacency]))
    s += "\n"
    return s
...
def create_training_instances_with_adj(input_files, adj_files, tokenizer, max_seq_length,
                              dupe_factor, short_seq_prob, masked_lm_prob,
                              max_predictions_per_seq, rng):
  """Create `TrainingInstance`s from raw text."""
  all_documents = [[]]; all_adjs = [[]]
...
def create_instances_from_document_with_adj(
    all_documents, all_adjs, document_index, max_seq_length, short_seq_prob,
    masked_lm_prob, max_predictions_per_seq, vocab_words, rng):
  """Creates `TrainingInstance`s for a single document."""
  document = all_documents[document_index]
...
def main(_):
  ...
  instances = create_training_instances_with_adj(
      input_files, adj_files, tokenizer, FLAGS.max_seq_length, FLAGS.dupe_factor,
