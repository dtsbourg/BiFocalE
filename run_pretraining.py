...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    tf.logging.info(model.get_attention_output())
    # (masked_lm_loss,
    ...
    (masked_lm_loss,
     ...
     masked_lm_predictions, label_ids) = get_masked_lm_explicit_output(
         bert_config, model.get_sequence_output(), model.get_embedding_table(),
         masked_lm_positions, masked_lm_ids, masked_lm_weights)
...
def get_masked_lm_explicit_output(bert_config, input_tensor, output_weights, positions,
                         label_ids, label_weights):
  """Get loss and log probs for the masked LM."""
  input_tensor = gather_indexes(input_tensor, positions)
  ...
  return  (loss, per_example_loss, log_probs, masked_lm_predictions, label_ids)
...
def main(_):
  ...
  validation_files = []
  for input_pattern in FLAGS.validation_file.split(","):
    validation_files.extend(tf.gfile.Glob(input_pattern))

  tf.logging.info("*** Input Files ***")
  for validation_file in validation_files:
    tf.logging.info("  %s" % validation_file)
  ...
  if FLAGS.do_eval:
    tf.logging.info("***** Running evaluation [VALIDATION] *****")
    tf.logging.info("  Batch size = %d", FLAGS.eval_batch_size)
    tf.logging.info(validation_files)
    ...
    output_eval_file = os.path.join(FLAGS.output_dir, "eval_results_val.txt")
    ...
    tf.logging.info("***** Running evaluation [TRAINING] *****")
    tf.logging.info("  Batch size = %d", FLAGS.eval_batch_size)
    ...
    output_eval_file = os.path.join(FLAGS.output_dir, "eval_results_train.txt")
    with tf.gfile.GFile(output_eval_file, "w") as writer:
      tf.logging.info("***** Eval results *****")
      for key in sorted(result.keys()):
        tf.logging.info("  %s = %s", key, str(result[key]))
...
if __name__ == "__main__":
  ...
  flags.mark_flag_as_required("validation_file")
